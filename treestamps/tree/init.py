...
@cache
def _version() -> str:
    """Return the installed treestamps version for generated file headers."""
    # Cached: every snapshot dump and WAL init emits a header, and a version
    # lookup scans metadata paths.
    try:
        return version("treestamps")
...
class TreestampsInit(TreestampsBase):
    ...
    def _serialize_program_config(self) -> dict:
        ...
        # NOTE: A program whose file selection depends on the treestamps
        # ignore & symlinks options must include them in its program config;
        # treestamps no longer records them itself.
        yaml = {}
        if self._config.program_config is not None:
            yaml[self._CONFIG_TAG] = _to_yaml_types(self._config.program_config)
        return yaml

    def _header_lines(self) -> tuple[str, ...]:
        """Return the comment lines that lead every generated file."""
        lines = (
            (
                f"@generated by treestamps {_version()} for "
                f"{self._config.program_name} — machine-written; do not edit."
            ),
    ...
    def _write_header(self, stream: TextIO) -> None:
        """Write the generated file header comment to a stream."""
        stream.writelines(self._header_lines())
